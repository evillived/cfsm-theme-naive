...
export interface TrafficProgressProps {
  ...
  /** 流量限制（字节），0 表示不显示 */
  trafficLimit: number
  /** 流量限制类型：up | down | min | max | sum | total（CFSM 的 traffic_calc_type） */
  trafficLimitType: 'up' | 'down' | 'min' | 'max' | 'sum' | 'total'
...
// 是否为双颜色模式（total 与 sum 都按上下行合计展示）
const isDualColorMode = computed(() => props.trafficLimitType === 'sum' || props.trafficLimitType === 'total')
...
const resolvedSingleColor = computed(() => {
  ...
  switch (props.trafficLimitType) {
    ...
    case 'total':
    default:
      // sum / total 类型走双颜色模式，这里作为 fallback
      return resolvedUploadColor.value
  }
})
