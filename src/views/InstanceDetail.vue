...
const data = computed(() => nodesStore.findById(serverId.value))

/**
 * 详情页数据加载。
 *
 * CFSM 要求详情页使用单服务器接口与单服务器订阅：
 * - `GET /api/server?id=` 拉取初始数据
 * - 延迟窗口只在 `/api/servers` 返回，因此再补一次列表数据
 * - WebSocket 订阅切到 `subscribe=<id>`
 */
async function loadDetail() {
  ...
  const apiBase = data.value?.apiBase ?? api.apiBases[0] ?? window.location.origin

  try {
    const server = await api.getServer(apiBase, id)
    nodesStore.applyServerDetail(apiBase, server)
  }
  catch (error) {
    console.error('[InstanceDetail] 加载服务器详情失败:', error)
  }

  // 延迟窗口仅在列表接口返回，补一次以保证 PingChart 有数据
  try {
    const { servers, stats, regionStats, sysConfig } = await api.getServers(apiBase)
    nodesStore.applyListResponse(apiBase, { servers, stats, regionStats, sysConfig })
  }
  catch (error) {
    console.error('[InstanceDetail] 补取服务器列表失败:', error)
  }
