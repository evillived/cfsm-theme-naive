...
- **延迟与丢包的展示位置**：延迟曲线与区间统计在详情页 `PingChart`；首页卡片（`NodeCard` 的「延迟」行）
  ...
  分级口径为延迟 `<100ms` 绿 / `<200ms` 橙黄 / `≥200ms` 红，丢包 `≤1%` 绿 / `≤10%` 橙黄 / `>10%` 红
  （取色见 `latencyHelper` 的 `gradeHex`：good=green、fair=**amber `#F5B21A`**、poor=tomato；「一般」
  刻意不用 orange `#F97316`，它与红的明度几乎相同、在浅色与毛玻璃背景上难以区分）；
  探测超时统一显示为「超时」。
- **首页与列表的丢包取「近 30 分钟平均」**（延迟仍是瞬时值）：`/api/servers` 提供的 `loss_*` 只代表最近
  一次探测轮次，单轮掉包就会跳到 50%、读数一直在跳，因此主题会逐台读取 `GET /api/history/all?hours=0.5`
  并取窗口内平均值。首页因此会比 `/api/servers` 多发出与机器数量相同的历史请求（gzip 后每台约 1 KB，
  按 5 分钟刷新一次，关闭「启用实时推送」后不再定时刷新）；平均窗口为 `LOSS_AVERAGE_HOURS`，
  数值保留 1 位小数，某条线路在窗口内没有样本时回落到后端标量。
